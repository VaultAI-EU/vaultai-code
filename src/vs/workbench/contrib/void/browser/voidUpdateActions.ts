...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of Vault Code, please download the latest version! [Vault Code](https://voideditor.com/download-beta)!'
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `Vault Code Error: There was an error checking for updates. If this persists, please get in touch or reinstall Vault Code [here](https://voideditor.com/download-beta)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	metricsService.capture(`Vault Code Update ${metricsTag}: Checking...`, {})
	const res = await voidUpdateService.check(explicit)
	if (!res) {
		const notifController = notifyErrChecking(notifService);
		metricsService.capture(`Vault Code Update ${metricsTag}: Error`, { res })
		return notifController
	}
	else {
		if (res.message) {
			const notifController = notifyUpdate(res, notifService, updateService)
			metricsService.capture(`Vault Code Update ${metricsTag}: Yes`, { res })
			return notifController
		}
		else {
			metricsService.capture(`Vault Code Update ${metricsTag}: No`, { res })
			return null
		}
	}
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'Vault Code: Check for Updates'),
		});
	}
